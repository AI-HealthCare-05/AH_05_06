...
async def test_closed_gate_does_not_build_a_sender() -> None:
    from ai_worker.main import _run_message_dispatch_loop

    with (
        patch(
            "ai_worker.main.Config",
            return_value=SimpleNamespace(SMS_PROVIDER=SmsProvider.SOLAPI),
        ),
        patch(
            "ai_worker.main.sms_dispatch_gate_open",
            return_value=False,
        ),
