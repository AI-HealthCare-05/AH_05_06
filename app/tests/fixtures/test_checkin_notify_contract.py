...
CONTRACT = ROOT / "docs" / "contracts" / "checkin-api-v1.md"
...
#: 계약 §4 의 표. 이 값을 바꾸려면 세 곳을 함께 바꿔야 한다.
NOTIFIES = {
    "taking": False,
    "uncomfortable": True,
    "missing": False,
...
class TestTheThreePlacesAgree:
    def test_the_contract_lists_every_answer(self) -> None:
        text = CONTRACT.read_text(encoding="utf-8")
        for key in NOTIFIES:
            assert key in text, f"계약이 {key} 를 말하지 않는다"
...
class TestMissingStaysQuiet:
    ...
    def test_the_p7_3_caption_no_longer_promises_a_notification(self) -> None:
        ...
        공통 캡션만 「선택 즉시 알림이 전송된다」로 남아 있었다.
        """
...
class TestTheScreenActuallySignals:
    ...
    def test_a_failed_signal_does_not_block_the_patient(self) -> None:
        ...
        source = CHECKIN_JS.read_text(encoding="utf-8")
        body = source[source.index("function signalPick") :]
        body = body[: body.index("\n  }")]
        assert ".catch(" in body, "신호 실패를 삼키지 않으면 화면이 깨진다"

    def test_the_screen_signals_every_pick_so_a_change_can_withdraw_the_alert(self) -> None:
        """알림 대상만 보내면 **철회가 안 된다.**

        「중단」을 골랐다가 「잘 먹고 있어요」로 바꿔도 뒤엣것을 안 보내면
        서버에는 중단 신호가 남는다. 저장하지 않고 창을 닫으면 의원이
        없는 문제를 쫓는다. 브라우저에서 눌러 확인한 것이다.

        알림을 만들지 여부는 서버가 정한다 — 화면이 거르지 않는다.
        """
        source = CHECKIN_JS.read_text(encoding="utf-8")
        body = source[source.index("function signalPick") :]
        body = body[: body.index("\n  }")]
        assert "notifyFor" not in body, (
            "화면이 알림 대상을 스스로 걸러 내고 있다 — 그러면 답을 바꿔도 앞 알림이 안 거둬진다"
        )
