...
class HistoryVisit(BaseModel):
    ...
    #: 환자가 **처음** 연 시각. 마지막이 아니다 — 원문 「열람 05-27」이 그것이다.
    guide_viewed_at: datetime | None
    #: 몇 장을 읽었나 — 원문 「(4장 중 2장)」의 앞 숫자. **장이 적힌 열람만
    #: 센다.** 링크로 처음 들어온 줄에는 아직 어느 장인지가 없고, 그것을 첫
    #: 장으로 쳐 주면 안 읽은 장을 읽은 것으로 센다.
    guide_pages_read: int = 0
    #: 열 수 있는 장이 몇인가. 화면이 숫자를 박지 않게 서버가 함께 준다 —
    #: 장이 늘거나 줄면 두 화면(S2-2 · D1-6)이 따로 놀지 않아야 한다.
