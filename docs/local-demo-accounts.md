...
| 문자 발송 | `[임시]` `SMS_PROVIDER=mock` — 실제로 안 나간다. 발송은 `ai-worker`(`--with-ocr-worker`)가 맡고, 첫 문자 직전에 그 진료의 **원본 문서를 지운다**(KEY-349 — mock 에서도 지우므로 문자가 나간 진료는 원본 이미지·OCR 원문을 다시 볼 수 없다). 삭제 실패는 `SOURCE_NOT_DELETED`(문자 상세 「원본 삭제 재시도」, KEY-362), 의원 예약 주소가 빈 소진·재진 문자는 `BOOKING_URL_MISSING`, 문자가 예약된 뒤 수신 거부로 바뀐 환자는 `SMS_OPT_OUT` 으로 보류된다. 처음부터 거부인 환자(합성 CSV 의 `문자수신동의=N`)는 문서 업로드·안내문 생성이 409 `SMS_OPT_OUT` 으로 먼저 막혀 문자까지 가지 않는다 (`app/services/dispatch_gate.py`). `solapi` 로 바꾸면 KEY-338 좁은문·승인 번호 목록이 추가로 걸린다 — README 「문자 발송」 표 |
