...
| 처방 | 자궁내막증 · 비잔 (계속) · 비잔정(디에노게스트) 2mg · 1일 1회 · **84일** | |
...
기준은 `develop`(`df9ee87`). **「없음」은 코드가 아직 없다는 뜻이고, 「막힘」은 코드는 있는데 지금 상태로는 안 돈다는 뜻이다.**

> **2026-08-21 저녁 갱신.** `#82`(KEY-54)가 업로드 표·경로를 붙이면서 3·4단계가 **열렸다.** 아래 표와 §4 는 그 뒤 기준이다. 직접 서버를 띄워 걸어 본 결과를 적는다.
>
> **2026-08-22 재확인.** 그 뒤 `#81`(KEY-109)이 들어와 기준을 `54693c4` → `df9ee87` 로 옮겼다. `#81` 은 프런트·계약 문서만 바꿔 **여정 9단계는 그대로다** — 아래 표의 근거를 한 줄씩 다시 재고 확인했다.
