...
> **판정 기준 커밋 `a6a9005` (`develop`, 2026-08-20 01:27)**

**이 문서는 그 시점의 스냅샷이고 이후 변화는 추적하지 않는다.** 검수 증적은
「그때 무엇이 사실이었나」가 값이다. 뒤에 무엇이 풀렸는지는 그때의 문서가
말한다 — 여기에 덧쓰면 판정 근거와 뒷이야기가 섞인다.
...
넷이 사슬이라 **기준 시점에는** E2E 를 태울 구간이 없었다. 아래는 그 앞 단계까지의
검수 증적이다.

**판정은 아직 뒤집히지 않았다.** 셋이 풀렸지만 `#40` 이 열려 있고, 그 화면이
부르는 `GET /visits/{visit_id}/ocr-job` 이 서버에 없다 — `KEY-133` 이 그 자리다.
즉 **사슬의 마지막 칸이 여전히 비어 있어 OCR 흐름은 아직 한 번도 끝까지 돌지
않았다.** 막힌 이유가 「앞이 안 끝나서」에서 「계약이 비어서」로 옮겨졌을 뿐이다.
...
| 🟠 | `#31` 리뷰 | 「못 읽은 항목도 행을 만드는가」·「저신뢰 임계값을 서버가 주는가」 미답 |
...
> **2026-08-20 19:48 — `#37` 은 병합됐는데 외래키는 아직 안 걸렸다.** 실측이다.
>
> ```
> Staff.hospital        ForeignKeyField   ✅
> Patient.hospital_id   BigIntField       ← 그냥 숫자
> Visit.hospital_id     BigIntField       ← 그냥 숫자
> ```
>
> 위험은 그대로고 **값만 싸졌다** — 걸 상대가 이제 있다.
...
> **2026-08-20 19:48 — 11개가 14개가 됐다.** 사슬도 그대로 있다(`#69`→`#50`,
> `#70`→`#68`). 여기에 스냅샷 때는 없던 것이 하나 더 붙었다 — **`#50` 과 `#67` 이
> 마이그레이션 9 번을 나눠 갖고 있다.** 파일 이름이 달라 git 이 충돌로 잡지
> 않고, aerich 는 `int(prefix)` 로만 정렬해 둘 다 조용히 실행하며, CI 는
> 마이그레이션을 아예 돌리지 않는다(테스트가 모델에서 스키마를 직접 만든다).
> **세 겹 전부 못 본다.**
...
| 7 | 프런트 자동 검사 (3-3) | 화면 회귀를 사람이 눈으로만 보고 있다 |
| 8 | 마이그레이션 번호 충돌 | 6절 위험 4 참고 — `#50` · `#67` 이 9 번을 나눠 갖고 있다 |
