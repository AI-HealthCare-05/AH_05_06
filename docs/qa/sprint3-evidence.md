...
> 작성 2026-08-19 · **갱신 2026-08-20 19:48** · 담당 권일준 · 리뷰어 유가은
> **판정 기준 커밋 `a6a9005` (`develop`, 2026-08-20 01:27)**

**이 문서는 기준 커밋 시점의 스냅샷이다.** 검수 증적은 「그때 무엇이 사실이었나」가
값이라, 뒤에 바뀐 것을 덮어써서 처음부터 그랬던 것처럼 만들지 않는다. 대신 바뀐
자리마다 취소선과 화살표로 **무엇이 언제 풀렸는지**를 나란히 남긴다.
...
| 조각 | 기준 커밋 시점 | 그 뒤 (2026-08-20 19:48) |
|---|---|---|
| OCR 모델 (`KEY-59`) | PR `#31` 승인 · **미병합** | ✅ 병합 |
| OCR API (`KEY-60`) | PR `#32` **변경 요청** | ✅ 병합 |
| 판독 확인 화면 (`KEY-62`) | PR `#40` 리뷰 대기 | 🟠 리뷰 완료 · 지적 4건 반영 · **계약 하나(`KEY-133`)에 막힘** |
| 필드 수정 연동 (`KEY-63`) | **미착수** — 위 셋에 막힘 | ✅ `#42` 병합 |
...
| | 어디 | 무엇 |
|---|---|---|
| ~~🔴~~ | ~~`#32` (KEY-60)~~ | ~~변경 요청 상태 — OCR API~~ → ✅ 병합 |
| ~~🔴~~ | ~~`#39` (KEY-34)~~ | ~~`API-1` 전화번호 검색 · `API-2` `doctor_id`~~ → ✅ `KEY-108` 로 둘 다 고침 · `#39` 병합 |
...
| 🟠 | `#31` 리뷰 | 「못 읽은 항목도 행을 만드는가」·「저신뢰 임계값을 서버가 주는가」 미답 → **`KEY-134` 가 이 답을 범위로 가져갔다**(한금준 · 기한 08-28) |
...
| ~~1~~ | ~~`#39` 의 `API-1`·`API-2`~~ | ✅ `KEY-108` — 둘 다 고침 |
| ~~2~~ | ~~`#31` 리뷰 질문 두 개에 답~~ | ✅ 자리 생김 — `KEY-134` 가 범위로 가져갔다 |
| **1** | `front-desk/visits` 계약 | `S1-1` 이 매일 처음 부르는 API. **여전히 계약에도 구현에도 없고, 이것을 맡은 일감도 없다** |
| **2** | 병원 격리 외래키 | `#37` 이 병합됐다 — **지금이 가장 싼 시점이고, 미루면 데이터가 쌓인 뒤에 걸어야 한다** |
| **3** | 프런트 자동 검사 (3-3) | 🟠 절반 열림 — `#64`(`KEY-117`)가 러너와 브라우저 껍데기를 세운다. **IIFE 로 감싼 화면 파일은 아직 부를 수 없어** 환자 전환·비동기 완료·실패 복구 회귀는 그다음이다 |
| **4** | 마이그레이션 번호 충돌 | 6절 위험 4 참고 — `#50` · `#67` 이 9 번을 나눠 갖고 있다. **병합 순서를 정하는 것만으로 끝난다** |
