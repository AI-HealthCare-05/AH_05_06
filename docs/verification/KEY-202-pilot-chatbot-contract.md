...
코드 기준은 최신 `develop` `6a2a0b4ad71592e5d2ff05caf7d7c4def9809f13`이다. Pilot 배포 버전은 별도로 확인한다.
...
- KEY-264의 Pilot 고정 OTP 좁은문은 `develop`에 병합됐고 두 실행 플래그를 사용한 재배포가 공유됐다. 다만 실제 고정 OTP 요청까지 도달해야 활성화를 PASS로 판정하며, 테스트용 쿠키나 DB 변경으로 우회하지 않는다.
...
| 항목 | 결과 |
|---|---|
| Pilot 의료진 로그인 | PASS: 합성 의료진 계정으로 Pilot 병원 화면에 진입함 |
| KEY-264 배포 전제 | 두 실행 플래그를 사용한 재배포가 팀에서 공유됐으나 서버 로그의 활성화 문구는 직접 확인하지 못함 |
| 환자 화면 진입 시도 | BLOCKED: 환자 화면 셸에는 도달했지만 현재 주소에 일회성 링크 자격값이 없어 OTP 컨텍스트를 불러오지 못함 |
| 고정 OTP `000000` | BLOCKED: `POST /patient-auth/otp/issue` 선행 단계에 도달하지 못해 검증하지 못함 |
| 마스킹 증적 | 템플릿 경로 `/patient_wireframe/html/otp.html`에서 자격값 없음 오류를 확인함. query·fragment·토큰·OTP·쿠키·질문/답변 원문은 기록하지 않음 |

| ID | 결과 | 확인 내용 |
|---|---|---|
| C01 | BLOCKED | 유효한 환자 세션이 없어 승인 안내 기반 질문을 실행하지 못함 |
| C02 | BLOCKED | 별도 미승인 검증 fixture와 유효한 환자 링크를 확보하지 못함 |
| C03 | BLOCKED | A/B 환자 세션을 확보하지 못해 컨텍스트 교차 검증을 실행하지 못함 |
| C04 | BLOCKED | 병원 간 검증 계정·링크 조합을 확보하지 못함 |
| C05 | BLOCKED | 챗봇 세션에 진입하지 못해 승인 밖 요청의 구조화 차단 결과를 확인하지 못함 |
| C06 | BLOCKED | 안내·챗봇 응답이 생성되지 않아 공개 필드와 중첩 값을 검사하지 못함 |
| C07 | BLOCKED | 환자 안내·챗봇 요청이 실행되지 않아 확인할 앱/nginx 로그와 이용 이벤트 구간이 없음 |
| C08 | BLOCKED | 일회성 링크 자격값이 없어 OTP 발급·검증과 승인 안내 화면 증적을 확보하지 못함 |

KEY-264 실패로 판정하지 않는다. 고정 OTP 검증은 링크 발급 이력이 없는 승인
합성 안내가 준비된 뒤 다시 실행한다. 기존 토큰 조회, DB 직접 수정, 공유 시연
진료 재시딩으로 우회하지 않았다. 화면 증적은 자격값이 없는 일반 오류 화면뿐이라
별도 이미지로 첨부하지 않고, 노출되지 않는 템플릿 경로와 판정 사실만 기록했다.
