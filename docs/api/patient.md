...
POST /checkins/{token}/signals
     { "answer_key": "stopped_side_effect",
       ...
       "client_sequence": 2 }        그 화면 안에서 몇 번째로 고른 것인가
...
| 상황 | 무엇으로 정하는가 |
|---|---|
| 같은 화면(`client_session_id` 가 같다) | **`client_sequence` 가 큰 쪽** |
| 다른 화면(새로고침·새 탭) | **서버에 닿은 차례** |

새로고침하면 순번이 1 부터 다시 시작하므로 화면끼리는 비교할 수 없다. 다만 새로고침은 사람 손 속도라 두 화면의 요청이 겹칠 일이 없어 도착 순서로 충분하다. **좁은 창에서는 순번을, 넓은 창에서는 도착 순서를 쓴다.**
