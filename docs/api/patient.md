...
| `guide.goals` | 확정 진단과, 이름이 일치하는 구조화 `prescription_set.disease`로 질환 판을 고른 뒤 담당 의사의 `lab_baseline`(없으면 의원 공통)을 같은 진료의 확정 OCR 필드와 `name`·`keywords`로 연결한다. 병존 진단은 두 판을 모두 사용하며 모든 별칭 중 최신 확정값을 고른다. `confirmed_at`이 없는 값은 제외하고, 기준선이 비어 있거나 연령별이면 현재 값만 표시하며 목표 계산·차트는 만들지 않는다. 기준선이 확정 OCR과 매칭되지 않으면 서버 경고 로그로 관측한다 |
| `guide.summary` | **확정 데이터로 짓는 고정 문장**(KEY-365). 진료 처방의 `prescription_set` 이름과 같은 세트의 `disease`(자궁내막증 / 다낭성 난소 증후군)와 확정 처방 항목의 약 이름(성분 괄호를 뗀 상표명)만 쓴다 — 「{질환명}으로 진료받으셨고, {약}을 처방받으셨어요.」, 약이 둘 이상이면 「{첫 약} 외 N개」, 약이 없으면 「{질환명}으로 진료받으셨어요.」. 안내 본문·LLM·의료 설명은 넣지 않는다. 세트를 못 찾으면 생략하고 화면은 요약 카드를 세우지 않는다 |
| `guide` | 승인 `medication` 최종 문구와 처방 항목. 전용 재진 계획 소스가 없어 현재 `next`는 생략. 문구의 「■ 소제목」 규칙은 아래 |
| `care` | 승인 `caution`·`emergency` 최종 문구. 일반 `messages`를 증상별 문의 기준으로 바꾸지 않으므로 현재 `ask`는 생략 |
| `life` | 승인 `life` 최종 문구. 「■ 소제목」이 있으면 소제목마다 한 축, 없으면 `생활관리` 한 축 |

**「■ 소제목」 — KEY-365.** 승인 문구 본문의 줄 맨 앞 `■ 소제목` 에서 본문을
나누고, 소제목은 카드 제목으로만 쓴다(소제목 줄은 문단으로 안 나간다). 첫 소제목
앞의 글은 아래 「소제목 없음」 자리로 간다. `■` 가 없는 본문(RAG 생성문, 의사 수정
문구, 옛 안내문)은 **손대지 않고** 그 자리에 통째로 들어간다. 소제목만 있고 문단이
없으면 카드를 세우지 않고, 같은 소제목이 두 번 나오면 한 카드에 모은다.

| 절 | 소제목 | 들어가는 곳 |
|---|---|---|
| `medication` | 소제목 없음 · `이 약을 복용하는 이유` | `guide.why` (「이 약을 왜 드시나요」) |
| `medication` | `복용 방법` | `guide.how` — 처방 용법 아래에 빈 줄로 붙는다 (「약별 복용 방법」) |
| `medication` | 그 밖의 소제목 (`복용 안내` 등) | `guide.blocks[]` `{t, p}` — 「약별 복용 방법」 다음, 「다음 방문 계획」 앞 |
| `caution` | 소제목 없음 | `care.blocks[]` 의 `주의사항` 카드 |
| `caution` | 소제목 (`흔하고 괜찮은 반응`, `진료 시 알려주세요` 등) | `care.blocks[]` 의 그 이름 카드 |
| `emergency` | — | **나누지 않는다.** `care.danger` 에 본문 그대로 (🚨 카드에 제목이 이미 있다) |
| `life` | 소제목 없음 / 소제목 | `life.axes` 의 `생활관리` 축 / 그 이름 축 |

- `guide.blocks` 는 선택 필드다. 짝 없는 복약지도 소제목이 없으면 키째 빠진다.
  **화면(`guide-api.js`)이 모르는 키를 계약 오류로 막으므로 web 과 app 을 같은
  배포에 올린다** (KEY-365 에서 응답 모양 고정 조항의 예외로 추가).
- `stat.why` 는 더 채우지 않는다. 복약지도 본문은 「이 약을 왜 드시나요」에만
  나온다 — 같은 문단이 현황·요약·왜 드시나요 세 카드에 반복되던 것을 없앴다.
...
- 쿠키가 없는 새 브라우저·시크릿 창·다른 기기에서 안내를 읽거나 D+7 양식을
  조회·저장하려면 OTP를 다시 확인해야 한다 (KEY-178).
- 세션은 발급 후 30분이 지나면 만료된다. 환자 안내 조회, D+7 조회·저장 모두
  유효한 세션이 필요하다. 세션이 없거나 만료·폐기된 경우
...
- 한 안내에는 D+7 응답 한 건만 저장한다. **같은 답을 다시 보내면 그때 그 줄을
  그대로 돌려준다**(`201`, KEY-335) — 두 번 누름과 응답 유실 뒤의 재시도가 여기로
  온다. 저장은 이미 됐는데 오류를 보여 주면 환자가 된 일을 실패로 본다.

  | 두 번째 요청 | 응답 |
  |---|---|
  | 저장된 답과 **같다** | `201` · 첫 저장 줄을 그대로 (`check_in_id` 도 같다) |
  | 저장된 답과 **다르다** | `409 CHECKIN_ALREADY_ANSWERED` · **먼저 저장된 값은 안 바뀐다** |

  같은 답인지는 `medication` · `pain.had` · `pain.score` · `pain.types` 넷으로
  가른다. 🚩 `pain.types` 는 **집합으로** 본다 — 화면이 체크박스라 고른 차례가
  그대로 실려 오는데, 순서까지 따지면 같은 답이 conflict 가 된다. `pain: null`
  (안 물어봄)과 `pain.had: false`(없다고 답함)는 **다른 답**이다.

  응답을 **고치는** 경로는 없다. 다른 답이 오면 조용히 덮지 않고 막는다 —
...
#### 안내문 한 장을 열었다 — KEY-256

```text
POST /api/v1/guides/{token}/views     환자 링크 자체가 접근 증명
{ "section": "caution" }              medication · caution · emergency · life · messages
204                                   본문 없음
```

안내문은 `GET /guides/{token}` 한 번에 통째로 내려가고, 환자 화면은 탭을
**브라우저 안에서만** 넘긴다. 그래서 서버에는 「열었다」한 줄만 남았고
「어디까지 읽었나」가 없었다 — 스탭은 다 읽은 환자와 첫 장만 열고 닫은 환자를
구별할 수 없다. 둘 다 「안내문 보셨어요?」에 「네」라고 답하지만 다음 진료 때
물을 것이 다르다.

- **본문에 원문이 없다.** 어느 장을 열었는지 하나뿐이다 — 이용 기록의 선을
  그대로 지킨다. 시각은 서버가 찍고, 누구인지는 토큰이 말한다.
- **`204` 다.** 화면이 받을 것이 없고, 이 호출이 실패해도 안내문은 계속
  읽혀야 한다 — 통계가 환자의 열람을 막으면 안 된다. 화면도 답을 안 기다린다.
- 승인 전 안내문에는 기록이 생기지 않는다(`record_guide_view` 의 기존 규칙).
- 모르는 `section` 은 `422` 다. 이 라우터는 봉투(`ContractRoute`) 면제
  태그(`patient-guides`)라 `400 INVALID_REQUEST` 로 안 바뀐다.
- 스탭이 보는 자리: S2-2 환자 이력 모달의 「열람 05-27 **(4장 중 2장)**」와
  D1-6 현황의 「안내문 읽음 4장 중 2장」. 분모는 **환자가 열 수 있는 장**
  넷이다(현황 · 복약지도 · 주의사항 · 생활관리). `emergency` 는 탭이 아니라
  주의사항 안에 실리는 문단이라 분모에 안 넣는다 — 못 여는 장을 넣으면 다
  읽어도 「5장 중 4장」이 된다.
...
| 부르는 쪽 | 부를 것 | 상태 |
|---|---|---|
| `GET /api/v1/guides/{token}` | `record_guide_view()` | **연결 완료** — KEY-170. 장 없이 「열었다」만 |
| `POST /api/v1/guides/{token}/views` | `record_guide_view(section=…)` | **연결 완료** — KEY-256. 탭을 넘길 때마다 |
...
- `submission_id`(UUID, 선택)는 **같은 물음을 두 번 보내지 않기 위한 열쇠**다
  ...
  환자 피드백(`POST /patient-feedback`)과 같은 모양이다. 열쇠는 **그 안내문
  안에서만** 유일해서 다른 안내문의 같은 열쇠와 안 부딪힌다.

  같은 열쇠가 다시 오면 **모델을 부르지 않는다.** 5분 안이면 그때 답을 그대로
  돌려주고(`response_ref` 까지 같은 값이다), 그 밖은 아래 셋 중 하나로 막는다.

  | 코드 | 언제 | 화면이 할 일 |
  |---|---|---|
  | `409 CHATBOT_ANSWER_IN_PROGRESS` | 앞의 요청이 아직 답하는 중 | **같은 열쇠로** 잠시 뒤 다시 |
  | `409 CHATBOT_ANSWER_EXPIRED` | 이미 답했는데 5분이 지나 되돌려 줄 답이 없음 | 열쇠를 버리고 **새로 묻기** |
  | `409 CHATBOT_SUBMISSION_CONFLICT` | 같은 열쇠에 다른 물음이 옴 | 열쇠를 버리고 **새로 묻기** |

  🚩 **답 본문은 표에 안 남는다.** 영속으로 남는 것은 `chatbot_submission` 의
  열쇠 digest · 물음 digest · 시각뿐이다. 답은 Redis 에 5분만 둔다 — 환자
  질문·답변을 영구 보관하지 않는 쪽으로 간다(2026-09-14, 이희진). 그래서 5분이
  지나면 **새로 답하지 않고 막는다**: 새로 답하면 이용 기록이 두 줄이 되어
  「같은 물음은 한 줄」이 깨진다.

  🚩 **열쇠 없는 요청은 멱등 보장이 없다.** 옛 화면·스크립트가 아직 안 보내므로
  당분간 그대로 통과시킨다. 필수로 돌리는 것은 보내는 쪽이 다 갈린 뒤의 일이다.
...
> 최초 결정 2026-08-21 · 구현 갱신 2026-09-15 KEY-238 (KEY-335 최종 저장 멱등성 유지)
...
저장은 **저장 시점에 새 `client_sequence`를 발급받아** 세 칸(`client_id` · `client_session_id` · `client_sequence`)을 함께 싣는다. KEY-335에 따라 안내당 최종 답은 한 건이며, 최초 성공 저장이 현재 신호를 보정한다. 이후 지연 신호는 이력에만 남고 확정 답을 되돌리지 않는다. 동일 답·통증·정규화된 메모의 재시도는 기존 결과를 반환하고, 다른 최종 답은 409다.
...
세 식별 필드는 함께 전달하거나 함께 생략한다(기존 클라이언트 호환). 신호는 세 필드 모두 필수다. 순번은 1~2^53-1 정수이며 동일 기기·순번의 다른 payload는 `CHECKIN_SIGNAL_CONFLICT` 409다. 서버는 안내 행 잠금과 locking read로 동시 요청을 직렬화한다.
...
5. **최종 저장이 안전망이다.** 신호가 실패했어도 저장 답으로 현재 상태를 보정한다. `notify`를 클라이언트 입력으로 신뢰하지 않으며 `stopped_side_effect`·`stopped_improved` 두 답만 확인 대상이다. `taking`·`missing`·`uncomfortable`은 확인 알림을 만들지 않는다.

실제 폼과 목업의 `notify`/`ask`는 같은 의미를 갖는다. 의료 안내 문구는 기존 승인 GuideSection을 재사용한다.
...
- 신호 보관 기간과 진료 차원의 해소 표시는 별도 정책이다. ACKNOWLEDGED를 해소로 사용하지 않는다.
