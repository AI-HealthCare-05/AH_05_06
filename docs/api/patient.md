...
POST /checkins/{token}/signals
     { "answer_key": "stopped_side_effect",
       "client_session_id": "…",     화면 하나를 가리킨다
       "client_sequence": 7 }        기기 하나에서 단조증가하는 번호
...
### 3.2 무엇이 「지금 답」인가 — 순서는 화면이 매긴 번호로 정한다

**받은 차례로 정하면 안 된다.** 첫 요청이 느리면(첫 연결·재시도·혼잡) 나중에 고른 답이 먼저 닿는다. 그러면 환자는 「잘 먹고 있어요」로 바꿨는데 서버의 지금 답은 「중단」이 되고, 의원은 없는 문제를 쫓는다. **이 계약이 막으려던 상황이 그대로 되살아난다.**

**`client_sequence` 가 큰 쪽이 나중이다. 그것 하나로 정한다.**

순번은 화면이 매기고 **기기 하나에서 단조증가한다** — 새로고침해도, 탭을 새로 열어도 다음 번호를 받는다(`localStorage`). 그래서 어느 화면에서 왔든 비교가 서고, 망이 어떻게 흔들려도 순서가 뒤집히지 않는다.

> **화면마다 1 부터 다시 시작하면 안 된다.** 새로고침 직전에 **이미 출발해 지연 중이던** 요청과 새 요청을 견줄 수 없게 된다. 서버가 「먼저 본 쪽이 앞」으로 정하는 것도 같은 이유로 안 된다 — 먼저 떠난 요청이 늦게 닿으면 오히려 나중으로 잡힌다.
>
> 이 경우는 유가은 님이 `#79` 재검토에서 재현해 주셨다.
>
> ```
> 화면 A   위험 답변 요청 출발 → 망에서 지연
> 새로고침
> 화면 B   정상 답변 도착 → 지금 답 taking
> 화면 A   지연된 위험 답변 도착   ← 순번이 작으므로 덮지 못한다
> ```

`client_session_id` 는 **어느 화면이 보냈는지 되짚을 때만** 쓴다. 판정에는 들어가지 않는다.
