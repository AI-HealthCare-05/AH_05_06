...
| 메서드 | 경로 | 권한 | 하는 일 |
|---|---|---|---|
| GET | `/api/v1/admin/hospital` | `admin` | 내 의원 정보 |
| PATCH | `/api/v1/admin/hospital` | `admin` | 의원 이름 · 대표번호 · 주소 · 예약 링크 수정 |

**`hospital` 이지 `hospitals` 가 아니다.** 한 배포에 의원은 하나다 — 의원마다
제 서버에 배포하는 폐쇄망 제품이라, 목록도 생성도 삭제도 없다. 복수형으로 두면
다음 사람이 그런 것을 얹을 자리가 있다고 읽는다.
