...
(KEY-322 · KEY-331). 나머지 관리자 화면(A1-5 문자 잔량)은
각자의 일감에서 확정한다.
...
`409` 는 **어느 의원이 쓰고 있는지 말하지 않는다.**

**그리고 여기서는 새어 나갈 것이 없다.** 케어온은 **의원 하나짜리 폐쇄망**이다 —
의원마다 자기 서버에 배포하고 DB 도 그 서버에 있다. 한 판에 의원이 하나이므로
이 `409` 가 알려 주는 것은 **그 의원 자신의 아이디**뿐이고, 그것은 이미
`GET /admin/staffs` 로 목록째 보인다.

> **`hospital` 표와 `hospital_id` 울타리를 멀티테넌트로 읽지 마라.** 그것은
> 방어이지 제품 요구가 아니다. `scripts/seed.py` 의 「격리의원」도 두 번째
> 고객이 아니라 **울타리를 재는 검사용 상대**다.
>
> 이 자리를 한 번 잘못 읽고 로그인에 의원 코드 칸을 더하는 일을 실제로 했다
> (KEY-324 · `#288`). 폐쇄망이라 전제가 성립하지 않아 중단했고, 구현은
> `feat/KEY-324-clinic-scoped-login` 가지에 남겨 두었다. **한 판에서 의원을
> 둘 이상 다루게 되는 날** 그 가지를 꺼낸다 — 그전에는 할 일이 아니다
> (2026-09-11).
...
| PATCH | `/api/v1/admin/hospital` | `admin` | 대표번호 · 주소 · 예약 링크 수정 |
...
**의원 이름은 못 바꾼다.** `HospitalUpdateRequest` 에 칸이 없다 — 배포 한 판의
정체라 시드가 정하고, 바꾸면 이미 나간 문자의 `{의원명}` 과 앞으로 나갈 것이
갈린다. 응답에는 들어 있다(화면이 「무엇을 고치는 중」인지 보여야 한다).

### 10.3 수정 — `PATCH /admin/hospital`

```json
{ "phone": "02-123-4567", "address": "서울시 ○○구 ○○로 12, 3층", "booking_url": "https://booking.example.com/dorothy" }
```

**화면은 셋을 늘 함께 보내고, 서버는 그 약속을 믿지 않는다.** 안 보낸 칸은
손대지 않고, 지우는 것은 **`null` 을 보낸 것**만이다. 화면이 셋을 함께 보내는
것은 한 폼에 셋이 같이 있어서이고(보고 있는 것이 곧 보내려는 전부), 서버가 안
보낸 칸을 지우면 화면 말고 다른 손님이 칸 하나만 보내는 순간 나머지가 사라진다.

**빈 몸(`{}`)은 거절한다** — 「셋 다 지워라」로도 「아무것도 안 바꾼다」로도
...
`hospital.updated_by` 에 마지막으로 고친 사람만 남는다 —
`MessageTemplate.updated_by` 와 같은 자리다. 별도 감사 이벤트 표는 만들지
않았다: 문자 문구(D2-5)도 같은 방식이고, 한쪽만 append-only 로 두면 두 화면이
같은 무게의 일을 다르게 기록한다. 의원 정보 변경을 A1-6 전체 로그에 세울지는
문자 문구와 **함께** 정할 일이다.
