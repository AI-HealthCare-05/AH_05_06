...
같은 까닭으로 **안 나간 것은 `limit` 에 잘리지 않는다.** 잘리는 것은 예정뿐이고, 잘리면 `truncated` 로 말한다 — 다 보이는 줄 알면 없는 것을 없다고 믿는다.

`total` 과 `window` 는 **세는 범위가 다르다** — 원문의 「전체 42」와 「이번 주 18」이다. 한 이름으로 뭉치면 화면이 42 를 「이번 주」라고 말하게 된다.

**스탭도 본다.** 이 화면이 잡으라는 것(잘못된 번호 · 문자 잔량)이 스탭이 손댈 일이라, 의사만 여는 자리로 두면 화면의 뜻이 없어진다.

**아직 없는 것** — 원문 할 일 칸의 넷 중 갈 데가 있는 것은 「번호 수정」 하나다. 시각 변경 · 즉시 발송 · 문자 충전은 API 도 없고, 그 아래에 문자를 실제로 보내는 **발송기 자체가 없다**. 문자 잔량을 담는 표도 없어 원문의 「⚠ 문자가 320통 남았습니다」 배너는 만들지 않았다. 그래서 `HELD` · `FAILED` 를 넣는 자리도 아직 없다 — 지금 실제 DB 에는 `SCHEDULED` 와 `CANCELED` 만 있다.
...
**아직 없는 것** — 원문의 「재승인」(번호를 고치고 다시 보낸다)은 발송기가 없어 만들지 않았다. 그리고 **아무도 `SENT` 를 만들지 않으므로 실제 DB 에서 이 목록은 비어 있다.**
