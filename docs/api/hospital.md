...
```jsonc
// 요청
{ "clinic_code": "clinic0001", "login_id": "staff01", "password": "…" }
...
**의원 코드를 함께 받는다** (KEY-324). 직원 아이디는 **의원 안에서만** 유일하다 —
두 의원이 `reception` 을 나눠 쓸 수 있고, 관리자가 아이디를 넣어 보며 남의 의원
계정 존재를 알아낼 수도 없다(§8.4).

코드 규칙은 직원 아이디와 같은 글자 집합이다 — `^[a-z0-9]{4,20}$`. 두 칸을 따로
받으므로 기존 아이디 규칙은 그대로다. 화면은 대문자로 쳐도 내려서 보낸다.

**틀린 코드와 틀린 아이디는 같은 답을 받는다.** 없는 의원 코드도 없는 아이디와
같은 길을 가 `401 invalid_credentials` 다 — 코드만 바꿔 가며 두드려 「그 의원이
있는가」를 알아낼 수 없다. 잠금 카운터도 **(의원, 아이디)** 단위라, 한 의원의
`reception` 이 잠겨도 다른 의원의 `reception` 은 멀쩡하다.
...
`409` 는 **로그인한 관리자의 의원 안에서만** 난다 (KEY-324). 아이디 유일성이
`(hospital_id, login_id)` 라 다른 의원이 같은 아이디를 써도 여기 안 걸리고,
그 의원에 무엇이 있는지도 알 수 없다.

> 전에는 `login_id` 가 전체에서 유일했다. 로그인이 `{아이디, 비밀번호}` 둘만
> 받아 병원을 알 수 없었기 때문이다(KEY-26 §4). 그래서 관리자가 아이디를 하나씩
> 넣어 보며 **남의 의원 아이디 존재를 알아낼 수** 있었다. 로그인이 의원 코드를
> 함께 받으면서(4절) 그 뿌리가 없어졌다.
