...
환자 D1 「나의 목표」는 확정 진단으로 질환 판을 고르고, 담당 의사의 기준선(없으면 의원 공통)을 같은 진료의 확정 OCR 필드와 `name`·`keywords`로 연결한다. 기준선이 비어 있거나 `by_age`이면 현재 값만 표시하고 목표 대비 계산은 하지 않는다. `confirmed_at`이 없는 미확정 값은 입력으로 사용하지 않는다.
