...
`GuideResponse` 를 돌려주는 모든 자리(생성·조회·제출·승인·철회·반려)가 `preview` 를 함께 준다. 스탭·의사 화면의 「환자 화면 미리보기」가 **나의 목표 · 처방받은 약 · 약별 복용 방법** 카드를 그리는 데 쓴다 — 이 셋은 `sections` 가 아니라 처방·검사 기준선에서 나오는 파생이다.
