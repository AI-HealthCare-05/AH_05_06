...
다른 의원이 쓰고 있어도 여기서 걸리는데, 그것을 알려 주면 남의 의원 계정
목록을 아이디 하나씩 물어 알아낼 수 있다.

**그래도 「어딘가 쓰인다」는 샌다.** 뿌리는 `login_id` 의 전역 유일성이고, 그것은
로그인이 병원을 알기 전에 일어나는 설계(KEY-26 §4)에서 온다 — 여기서는 못
고친다. 의원별 유일로 옮기는 것은 **KEY-324** 다. 그때 이 `409` 를 의원 범위로
좁히고 이 문단도 걷는다. 그전까지는 이대로 둔다 (2026-09-10 안전 리뷰 결정).
