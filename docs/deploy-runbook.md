...
트리**로 이미지를 빌드한다(`BUILD_CONTEXT_PATHS` 가 `app`·`ai_worker`·`frontend`
를 그대로 담는다). 가지를 안 보고 돌리면 **그 순간 열려 있던 가지가 서버로
나간다.**

    git fetch origin               # 이것부터. 없으면 벌어진 것이 안 보인다
    git branch --show-current      # develop 이어야 한다
    git status -sb                 # origin/develop 과 안 벌어졌는지

2026-09-11 에 실제로 물렸다. 저장소가 `qa/2026-09-11` 에 서 있는 채로 배포해서
**리뷰 반영 전 코드**와 **옛 이름을 단 마이그레이션**(`52_…key331_hospital_info`
— 그 사이 `54_…` 로 옮긴 파일이다)이 올라갔다. `up -d` 직전에 알아채 멈췄지만,
띄웠으면 A1-3 직원 수정이 좁은 칸에 긴 값을 넣어 500 이 났을 것이다.
...
**병합될 때까지 그 파일의 이름도 내용도 바꾸지 않는다**(2026-09-14 기준 53·54).
번호가 겹쳐 보여도 마찬가지다 — 옮기는 순간 장부와 갈린다. 번호와 스냅샷을
맞추는 정상 절차는 **KEY-333** 에서 정한다.

### 예외 복구 — 이미 갈렸을 때만

**여기부터는 정상 경로가 아니다.** 위를 못 지켜 장부와 파일이 이미 갈린 뒤에만
쓴다. 네 조건이 **모두** 맞아야 한다.

1. 옛 파일과 새 파일이 **순수 이름 변경**이다 — 내용이 같다. `shasum -a 256` 으로
   두 파일을 떠서 **직접 대조한다.**
2. 그 마이그레이션의 **스키마가 서버에 실제로 적용돼 있다**(칸·표가 이미 있다).
3. **DB 백업을 먼저 뜬다.**
4. 고친 사실을 **KEY-6 에 기록한다** — 다음 사람이 장부를 의심할 때 볼 자리다.

```sql
UPDATE aerich SET version = '<새 이름>' WHERE app = 'models' AND version = '<옛 이름>';
```

`app = 'models'` 를 빼지 마라. 장부는 앱별로 줄을 갖는다.

### 🚩 버전은 **두 곳**이 맞아야 한다

스크립트가 묻는 버전은 **빌드해서 밀 태그**다. 서버가 무엇을 받을지는
`envs/.prod.env` 의 `APP_VERSION`·`AI_WORKER_VERSION`·`WEB_VERSION` 이 정한다
(compose 가 이미지 이름을 그 값으로 짓는다). **올리기 전에 이 파일부터 고친다.**
...
2. **Pilot/staging에서 승인된 테스트 번호로 실제 문자 1건 수신.**
   ...
   **와** `--otp-confirm-solapi-prod` 실행 플래그를 **둘 다** 요구한다(4-3-1의
   ...
   ```python
   # app/services/dispatch_gate.py — _source_deletion_state() (KEY-349)
   행               삭제 기록   파일       판정
   없음             —          —          통과, 지울 것 없음
   있음             없음       있음/없음  통과, 이 문서를 지울 차례로 보고
   있음             있음       없음       통과, 지울 것 없음(이미 끝남)
   있음             있음       있음       막는다 — 기록과 실제가 어긋난다
   ```

   원본 행이 있는데 삭제 기록이 아직 없으면(가장 흔한 경우) 게이트는 막지
   않는다 — `dispatch_message()`가 그 자리에서 실제로 지우고 확인·기록까지
   끝낸 뒤에만 발송이 이어진다(KEY-349). 게이트가 막는 것은 **기록과 실제가
   어긋나는 경우뿐**이다(삭제했다고 기록했는데 파일이 남아 있음). 그러니
   "예약 문자를 세어 보고 켠다"는 이제 "다시 나갈 때 원본 삭제가 한 번씩
   더 일어난다"는 뜻이지, 전부 막힌다는 뜻이 아니다.
   ...
   **Pilot 실검증 상태(2026-09-15): 미실행.** 배포 권한이 있는 담당자가
   아래를 확인하고 PR에 결과를 기록한다.

   1. 좁은문을 닫은 채 `ai-worker`를 기동해 예약 문자가 `SCHEDULED`로
      유지되고 발송기가 만들어지지 않는지 확인한다.
   2. 같은 상태에서 OCR·안내 생성 작업이 계속 처리되는지 확인한다.
   3. `docker compose stop ai-worker` 후 프로세스가 제한 시간 안에 정상
      종료되고 재기동할 수 있는지 확인한다.
   4. 확인 뒤 위 명령으로 좁은문을 다시 닫고 워커 로그를 보존한다.
...
1. **쌓인 건수·수신 번호를 먼저 센다.** 얼마나 밀렸는지, 그중 승인
   ...
   후자는 다시 열어도 `RECIPIENT_NOT_APPROVED`로 계속 막히니 걱정할
   대상이 아니다.
2. **승인 목록 안의 번호 중, 그사이 내용이 달라졌을 수 있는 것을
   확인한다.** 좁은문이 닫혀 있던 동안 안내문이 수정·반려됐거나 원본
   문서가 새로 올라왔다면, 그 문자는 다시 열었을 때 다른 게이트
   (`NOT_APPROVED`·`SAFETY_CHECK_FAILED`)에 걸릴 수 있다 — 게이트
   자체가 그 시점의 최신 상태를 다시 읽으므로 별도 조치는 필요 없지만,
   "왜 안 나갔지"로 헷갈리지 않게 미리 안다. `SOURCE_NOT_DELETED`는
   KEY-349 이후 별도로 걱정할 필요가 없다 — 삭제 기록이 아직 없으면
   막는 게 아니라 그 자리에서 자동으로 지우고 넘어간다. 이 상태로
   보류되는 경우는 삭제·확인·기록 중 하나가 실제로 실패해서
   재시도가 소진된 것뿐이다 — 그때는 저장소(파일 권한·디스크 상태)를
   먼저 본다.
3. **문제 원인이 해소됐는지 먼저 확인한 뒤에만 켠다.** 좁은문을 닫았던
   이유(오발송·설정 오류 등)가 그대로면, 다시 열자마자 같은 사고가
   밀린 건수만큼 반복된다.
4. **켠 직후 몇 분간 `ai-worker` 로그와 발송 이력(S2-4)을 지켜본다.**
   밀린 문자가 한꺼번에 나가는 동안 실패·보류 비율이 평소와 다르면
   바로 다시 끈다.
