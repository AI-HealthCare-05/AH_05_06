...
(function () {
   ...
   * 하나가 됐다 — 묶음이 넷에서 둘로 줄었다. 씨앗 처방도 여덟에서 넷으로
   * 줄어(KEY-262) 줄이 넷이다. 둘 다 펴도 「기타」가 한참 남는다.
   *
   * **다만 줄 수는 이제 고정이 아니다** — 의사가 처방을 만들 수 있다(KEY-255).
   * 많이 만들면 다시 밀릴 수 있고, 그때는 여기 셈이 아니라 레일 자체를
   * 스크롤되게 두는 것이 맞다.
   *
  ...
  /* **실제로 나가는 글** — KEY-258 · 와이어프레임 D2-1 「환자가 받게 될 안내문」.
   *
   * 고칠 수 있게 해 놓고 **어떻게 나가는지 볼 길이 없었다.** 저장만 되고,
   * 그 글이 환자 안내문에서 어떤 모습인지는 진료를 하나 열어 봐야 알았다.
   *
   * ## 세로로 붙인다 — 셋째 칸이 아니다
   *
   * 원문 D2-2 의 두 층을 한 칸으로 합친 것이 이 화면의 결정이다(위 주석).
   * 옆에 칸을 하나 더 세우면 그 결정을 되돌리는 셈이고, 재 보니 편집 칸이
   * 860px 에서 274px 로 줄어 한 줄이던 주의 문구가 세 줄이 된다.
   *
   * 그리고 원문에서도 미리보기는 셋째 칸이 아니라 **D2-1 「환자가 받게 될
   * 안내문」** 이다 — 고치기 위에 뜨는 것이 아니라 아래 놓인 결과다.
   *
   * ## 🚩 범위를 말한다
   *
   * 이 화면에는 **진료가 없다.** 그래서 두 가지를 모를 수밖에 없다 —
   *
  ...
  /* 한 장 — 왼쪽에서 고른 것 하나만 선다. 처방이 한꺼번에 펼쳐져 있으면
