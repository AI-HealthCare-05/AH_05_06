...
(function () {
  ...
  /* 고르는 즉시 의료진에게 알린다 — 와이어프레임 P7-2·P7-4·P7-5 (KEY-138).
     저장할 때만 보내면 **고르고 창을 닫은 환자를 놓친다.** 그 사람이 곧 끊은
     환자이고, 폼을 끝까지 채울 가능성이 가장 낮은 환자다.

     **고른 것은 늘 보낸다.** 알림 대상인지는 서버가 정한다.

     처음에는 알림 대상(`notify: true`)일 때만 보냈는데, 그러면 **철회가 안
     된다.** 「불편해서 중단했어요」를 골랐다가 「잘 먹고 있어요」로 바꾸면
     뒤엣것은 안 보내지므로 서버에는 중단 신호가 남는다. 저장하지 않고 창을
     닫으면 의원은 없는 문제를 쫓는다. 브라우저에서 눌러 확인한 것이다.

     늘 보내면 신호가 덮어써져 알림이 거둬진다. `가끔 놓쳐요` 를 보내도 환자에게
     연락이 가지 않는다 — 서버가 알림을 만들지 않기 때문이다(P7-3).

     둘을 더 지킨다.

       ① **실패해도 환자를 막지 않는다.** 알림은 의원 쪽 편의다 — 못 보냈다고
          제출을 못 하게 하면 답 자체를 잃는다. 저장이 최종 보루로 남아 있다
       ② 오류를 화면에 띄우지 않는다. 환자가 할 수 있는 일이 없고,
          「전송 실패」는 자기 답이 안 갔다는 뜻으로 읽힌다 */
  function signalPick(key) {
    checkinApi.signal(token, key).catch(function () {
      /* 삼킨다 — 위 ①·②. 저장 때 `notify` 가 다시 간다. */
    });
  }
