/* ⚠️ **이 파일은 어떤 화면도 안 싣는다** — KEY-281.
 *
 * `guide.html` 이 싣는 것은 이름만 비슷한 `patient_wireframe/js/guide.js` 다.
 * 살아 있는 환자 챗봇은 `patient_wireframe/js/chat.js` 이고, 중단·늦은 조각·
 * 다시 시도는 `tests/key281-chat-abort-on-real-path.test.js` 가 그쪽에서 잰다.
 *
 * **아직 안 지운다.** KEY-233 이 「삭제 후보는 이희진 님 확인 전까지 실제로
 * 지우지 않는다」고 정해 두었다. 처분이 정해지면 이 머리말과 함께 없앤다.
 *
 * 이 파일을 고쳐도 사용자에게는 아무 일도 안 일어난다. 고칠 것이 있으면
 * `patient_wireframe/` 쪽을 본다.
 */
