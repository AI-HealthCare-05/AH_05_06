...
var catalogApi = {
  sets: function () {
    if (MOCK) return Promise.resolve(MOCK_PRESCRIPTION_SETS.map(mockSetListRow));
  ...
  saveSet: function (id, plan) {
    ...
    return request("/prescription-sets/" + encodeURIComponent(id), { method: "PUT", body: plan });
  },
};
...
function mockSetSeed() {
  ...
  return MOCK_PRESCRIPTION_SETS.map(function (row) {
    return {
      ...
      phase: row.name.indexOf("(처음)") !== -1 || row.name.indexOf("초진") !== -1 ? "FIRST" : "CONTINUE",
      days_mode: "DAYS",
      days_per_pack: null,
      emr_code: null,
      revisit_note: null,
...
function mockSetDetail(id) {
  return new Promise(function (resolve, reject) {
    setTimeout(function () {
      ...
      if (!mine) return reject(new ApiError("PRESCRIPTION_SET_NOT_FOUND", 404, {}));
