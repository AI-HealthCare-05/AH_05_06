 ...
 *
 * **의사는 스탭이 하는 모든 것을 한다** (2026-09-08 회의 결정, KEY-311). 환자를
 * 더하고, 안내문을 만들고, 현황을 본다. 승인·반려·기준선은 의사만 하므로 의사가
 * 스탭을 **포함**한다.
 *
 * 서버는 이미 그 정책이었다 — `app/core/rbac.py` 의 권한표에서 `PATIENT_READ` ·
 * `PATIENT_WRITE` · `OCR_UPLOAD` · `GUIDE_DRAFT` · `SMS_SEND` 가 전부
 * `{STAFF, DOCTOR}` 다. 막고 있던 것은 이 함수 하나뿐이었다.
 *
 * **`landingFor` 에서 파생하지 않는다.** 전에는 「이 계정의 첫 화면이 곧 이
 * 화면인가」로 물었는데, 그러면 착지점과 권한이 한 낱말이 된다 — 의사의 첫
 * 화면이 `/doctor.html` 이라는 이유만으로 현황이 닫혔다. 사용자가 본 「안내문·
 * 현황 탭이 안 눌린다」가 그것이다. 두 물음은 다르다:
 *
 *   · 로그인하면 어디에 내려놓는가 — `landingFor` (의사는 승인이 첫 일, KEY-269)
 *   · 일부러 찾아오면 볼 수 있는가 — 여기
 *
 * 착지 규칙은 `login.js` 가 그대로 지키므로 KEY-269 는 살아 있다.
 *
 * 어드민 **권한만** 가진 계정은 여전히 돌려보낸다 — 그 계정은 서버에서도
 * `PATIENT_READ` 가 없어 목록을 부르는 첫 걸음부터 403 이다. 보내 놓고 막느니
 * 제 화면으로 보낸다.
 *
