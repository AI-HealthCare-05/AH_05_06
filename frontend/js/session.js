 ...
 * 채로 들어오는 길을 못 막는다.
 *
 * **`landingFor` 에서 파생한다** — 「이 계정의 첫 화면이 곧 이 화면인가」. 규칙을
 * 두 번 적지 않으므로, 밀어낸 사람을 다시 여기로 보내는 무한 튕김이 원리상
 * 불가능하다. `doctor` 롤이 있으면 `landingFor` 가 `/doctor.html` 을 주므로
 * 「doctor 면 언제나 `/doctor.html`」(KEY-269)이 그대로 따라온다.
 *
 * 서버 차단이 아니다 — 의사는 스탭 API 권한을 포함해 호출이 403 이 나지 않는다.
 * 잘못 온 사람을 제 화면으로 보내는 화면단 처리이고, 실제 권한 검증은 서버가
 * 그대로 한다(KEY-9). */
function showsPatientList(roles) {
  return landingFor(roles) === "/patients.html";
}
