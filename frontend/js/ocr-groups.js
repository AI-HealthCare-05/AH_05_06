...
/* 둘 다 아직 못 채우는데, **막힌 곳이 서로 다르다.** 화면 문구를 뭉뚱그려
   ...
   만들어야 하는 줄 안다. 하나는 표가 이미 있다.
     ...
     ③ 이전 값 유지 — 값은 `ocr_field` 에 **이미 쌓여 있다.** 꺼내 오는
...
var GROUPS_WITHOUT_SERVER = [
  {
    ...
    title: "이전 값 유지",
    note: "이번 미시행",
    needs: "지난 진료의 확정 판독 값을 꺼내 오는 화면 연결. API(KEY-246)는 완성됐다",
    saying:
      "이번에 안 한 검사의 지난 값을 여기 흐리게 세웁니다 — 값은 이미 저장돼 있고, 아직 이 패널에 연결되지 않았습니다",
  },
  {
    key: "checks",
    title: "확인 항목",
