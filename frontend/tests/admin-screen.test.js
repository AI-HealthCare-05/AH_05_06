...
/* **처음에는 일곱이 다 「화면 없음」이었다.** 어드민에 데이터를 줄 API 가
   ...
   KEY-321 이 `A1-1`(직원 목록) · `A1-2`(직원 추가)에, KEY-322 가
   `A1-6`(전체 로그) · `A1-7`(한 건 시간 흐름)에, KEY-331 이 `A1-4`(의원
   정보)에 API 를 붙였다. 그래서 재는 것을 바꾼다: 「전부 3이다」가 아니라
   **「3인 것은 3이라고 말하고, 3이 아닌 것은 갈 곳이 있다」**. 값을
   지어내지 않았는지는 그대로 잰다. */
const LIVE_ADMIN_FRAMES = ["A1-1", "A1-2", "A1-4", "A1-6", "A1-7"];
