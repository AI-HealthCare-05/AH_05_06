...
test("**처방 이름은 잠근다** — 지난 진료기록이 그 이름으로 가리킨다", () => {
  ...
  assert.match(code, /"f-name",\s*"처방"/, "이름 칸이 통째로 사라졌다");
  ...
  assert.match(code, /지난 진료기록이 이 이름으로/, "왜 못 바꾸는지 화면이 말하지 않는다");
});
