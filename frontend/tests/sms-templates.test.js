...
test("UTF-8 로 세면 보낼 수 있는 문구를 못 보낸다고 한다", () => {
  ...
  assert.ok(
    Buffer.byteLength(forty, "utf8") > SMS_LIMIT,
    "UTF-8 로 세면 장문이라 잘못 말한다",
  );
});
...
test("변수를 이름으로 찾는다", () => {
  ...
  assert.deepStrictEqual(
    templateVariables("[{의원명}] {환자명}님 {링크}").join(),
    "의원명,환자명,링크",
  );
...
test("링크는 지울 수 없다", () => {
  ...
  const said = templateProblem(item, "{환자명}님, 오늘 어떠세요?", [
    "환자명",
    "링크",
  ]);
  ...
  assert.ok(
    said.indexOf("열 곳이 없어집니다") !== -1,
    "왜 막는지 적지 않으면 고장으로 읽힌다",
  );
});
...
test("멀쩡한 문구는 막지 않는다", () => {
  ...
  assert.strictEqual(
    templateProblem(item, "{환자명}님 잘 지내세요? {링크}", ["환자명", "링크"]),
    "",
  );
});
...
test("회차 이름을 다시 짓지 않는다", () => {
  ...
  assert.strictEqual(
    templateSaying("CHECK_D7"),
    "일주일 뒤 확인",
    "js/message-words.js 것을 쓴다",
  );
  assert.strictEqual(templateSaying("RUN_OUT"), "소진 임박");
  assert.strictEqual(
    templateSaying("REVISIT"),
    "재진 안내",
    "회차가 아니라 그 표에 없다",
  );
  assert.strictEqual(
...
test("문자 어휘를 여기서 베끼지 않는다", () => {
  ...
  for (const word of ["진료 안내문", "소진 임박", "보름 뒤", "한 달 뒤"]) {
    assert.ok(
      code.indexOf(word) === -1,
      `「${word}」를 여기 다시 적었다 — js/message-words.js 것을 쓴다`,
    );
  }
});
...
test("목업 기본 문구가 서버와 같은 글이다", async () => {
  ...
  assert.ok(
    page.items.every((row) => row.is_default),
    "안 고쳤으면 전부 기본값이다",
  );
...
test("원문이 장문이라 한 것만 장문이다", async () => {
  ...
  const page = await api.catalogApi.templates();
  const long = page.items
    .filter((row) => api.smsLength(row.body).long)
    .map((row) => row.kind);

  assert.deepStrictEqual(
    long.join(),
    "REVISIT",
    "원문이 재진 안내 하나만 ⚠ 장문(LMS)으로 표시한다",
  );
});
...
test("고치면 기본값이 아니게 되고 되돌리면 돌아온다", async () => {
  ...
  const before = (await api.catalogApi.templates()).items.filter(
    (r) => r.kind === "CHECK_D7",
  )[0];
  const after = (
    await api.catalogApi.saveTemplate("CHECK_D7", "{환자명}님 {링크}")
  ).items.filter((r) => r.kind === "CHECK_D7")[0];
  const back = (await api.catalogApi.resetTemplate("CHECK_D7")).items.filter(
    (r) => r.kind === "CHECK_D7",
  )[0];
...
test("저장 전에 화면이 먼저 잰다", () => {
  ...
  const save = code.slice(
    code.indexOf("function saveTemplates"),
    code.indexOf("function revertTemplate"),
  );

  assert.ok(
    save.indexOf("templateProblem(") !== -1,
    "눌러 보고서야 아는 것보다 낫다",
  );
...
test("하나라도 막히면 아무것도 안 보낸다", () => {
  ...
  assert.ok(
    checkAt !== -1 && sendAt !== -1 && checkAt < sendAt,
    "반만 저장되면 어느 것이 들어갔는지 모른다",
  );
});
...
test("고칠 수 없는 문자도 보인다", () => {
  ...
  const panel = code.slice(
    code.indexOf("function templatesHtml"),
    code.indexOf("function detailHtml"),
  );

  assert.ok(
    panel.indexOf("system_body") !== -1,
    "무엇이 나가는지는 알아야 한다",
  );
  assert.ok(panel.indexOf("수정 불가") !== -1);
  assert.ok(
    panel.indexOf("sms__fixed") !== -1,
    "읽는 자리가 입력칸처럼 보이면 안 된다",
  );
});
...
test("만든 묶음만 눌린다", () => {
  ...
  assert.strictEqual(
    rail.RAIL_GROUP_READY.baseline,
    true,
    "검사 기준선(D2-4)도 열린다",
  );

  /* 안내문(D2-1·D2-2)은 여기 없다 — 줄 하나로 들어가는 자리가 아니라
     갈래 밑에 장이 그대로 서고, 장 하나를 고르면 오른쪽이 그것만 편다. */
  assert.strictEqual(rail.RAIL_GROUP_READY.guide, undefined);
  assert.deepEqual(
    rail.RAIL_GROUPS.filter(function (row) {
      return row.section === "guide";
    }),
    [],
    "안내문 갈래에 묶음 줄이 서면 목록과 같은 자리를 둘이 가리킨다",
  );
});
...
test("설정 화면이 규칙 파일을 싣는다", () => {
  ...
  assert.ok(
    markup.indexOf("message-words.js") !== -1,
    "회차 이름이 거기서 온다",
  );
});
