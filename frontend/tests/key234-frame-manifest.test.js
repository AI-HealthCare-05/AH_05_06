...
/* **대상을 여기에 적어 둔다.** 「needsGuideScreen 으로 거른 것이
   ...
   **지금 9개다.** KEY-235 에서 12로 줄었고, KEY-321 이 `A1-2`(직원 추가)를,
   KEY-322 가 `A1-6`·`A1-7`(감사 로그)을 빼면서 셋 더 줄었다 — 다 실제로
   도는 화면이 생겼다.
   팀 계획 문서(`docs/work-packages.md`)는 20으로 적혀 있었는데, 그 숫자는 구조
   진단 §6.2 의 **목표치**(완전 36 · 조회 8 · 안내 20)에서 왔다. KEY-235 에서
   문서 쪽을 실측값으로 고쳤다.

   17 → 12 로 줄면서 빠진 다섯 — `S1-14`·`D1-4`·`D1-6`·`P5-1`·`P5-2`.
   다 **화면이 생겨** 안내할 자리가 아니라 쓰는 자리가 됐다. 앞서
   `S2-1~S2-4`·`D2-1~D2-5` 가 같은 이유로 빠졌다.

   🚩 **화면 조각이 생겼다고 다 빠지지는 않는다.** `P1-3` 은 만료·폐기 화면이
   그려지는데도 3 으로 남았다 — 「새 링크 요청하기」가 새 토큰을 만들어 놓고
   아무 데도 안 넘겨(발송 호출이 없다) 환자는 못 받는데 화면은 성공으로
   답한다. **거짓 성공이 화면 없음보다 나쁘다.** `D1-7`·`P1-5` 도 같은 결이다.
   프레임이 말하는 일을 사람이 끝까지 못 하면 안내 화면이 맞다. */
const GUIDE_SCREEN_FRAMES = [
  "A1-3", "A1-5",
  "D1-7",
  "P1-3", "P1-5", "P8-1", "P8-2", "P9",
  "S1-10",
];
