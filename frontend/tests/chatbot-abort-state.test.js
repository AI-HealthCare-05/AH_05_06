/* ⚠️ **이 검사는 아무도 안 싣는 파일을 잰다** — KEY-281.
 *
 * `frontend/js/guide.js` 는 고아다. 그래서 여기가 초록이어도 **사용자 경로가
 * 안전하다는 뜻이 아니다.** 실제로 그 사이에 계약 하나가 실제 화면에서 조용히
 * 깨져 있었다 — 「다시 시도」가 치던 글자를 지웠다.
 *
 * 살아 있는 경로를 재는 것은 `key281-chat-abort-on-real-path.test.js` 다.
 * 이 파일은 고아 파일의 처분이 정해질 때 함께 없앤다.
 */
