...
test("**휴대폰은 화면에 전부 보인다** — 가리면 번호를 EMR 에서 다시 찾는다", () => {
  /* 전에는 가운데를 `****` 로 가렸다(와이어프레임도 그렇게 그렸다). 그런데 이
     번호를 보는 사람은 그 환자를 방금 접수한 의원의 스탭이고, 안내 문자가 안
     가면 전화를 걸어야 하는 사람이다. 가려 두면 확인하러 EMR 을 따로 열게 되고,
     그 사이에 옮겨 적다 틀린다.

     **로그·오류 응답의 마스킹은 이것과 다른 문제다** — KEY-48 이 요구하는
     보안 사항이고 그대로 둔다. 화면은 권한 있는 사람이 보고, 로그는 누가 볼지
     모른다. */
  assert.equal(api.formatPhone("01044524085"), "010-4452-4085");
  assert.ok(!api.formatPhone("01044524085").includes("*"), "화면에서 번호를 가리고 있다");
});

test("끊어 쓰는 것은 읽기 위해서다 — 자릿수가 달라도 죽지 않는다", () => {
  assert.equal(api.formatPhone("0212345678"), "021-234-5678", "열 자리도 끊어 준다");
  assert.equal(api.formatPhone("010-4452-4085"), "010-4452-4085", "이미 끊긴 것을 두 번 끊지 않는다");
  assert.equal(api.formatPhone(""), "");
  assert.equal(api.formatPhone(null), "");
  assert.equal(api.formatPhone("123"), "123", "모르는 모양은 그대로 둔다");
});
