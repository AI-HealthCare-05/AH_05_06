...
test("**기기 화면을 흉내낸다** — 카드 몇 장이 아니다", () => {
  ...
  /* **KEY-286 뒤로는 제 목업이 아니다.** 환자 자신의 스타일시트를 신은
     iframe 이라, 모양을 이 저장소가 두 벌로 들고 있지 않다. */
  assert.ok(html.includes('class="pv"'), "기기 틀이 없다");
  assert.ok(html.includes("<iframe"), "환자 화면을 그대로 신지 않는다");
  assert.ok(html.includes("patient_wireframe/css/guide.css"), "환자 스타일시트를 안 신는다");
  assert.ok(!html.includes("ph__block"), "옛 목업이 남아 있다");
});

test("**탭이 넷이다** — 다섯이 아니었다 (환자 원문)", () => {
  const { guidePreviewHtml } = box();
  const html = guidePreviewHtml(SECTIONS, "medication");

  /* 환자 렌더러의 `TABS` 가 넷이다. 옛 미리보기는 다섯을 그렸고 그중
     「챗봇」은 환자 탭 바에 없다 — 「환자가 받는 그대로」가 거짓이던 자리다. */
  for (const label of ["현황", "복약지도", "주의사항", "생활관리"]) {
    assert.ok(html.includes(label), `${label} 탭이 없다`);
  }
  assert.ok(!html.includes("챗봇"), "환자 탭 바에 없는 것을 그렸다");

  /* 환자는 「생활관리」라 부른다 — 의료진 화면의 「생활지도」와 다르다 */
  assert.ok(!html.includes("생활지도"), "환자 화면에 의료진 쪽 이름을 썼다");

  /* 탭이 본문보다 위에 온다 */
  assert.ok(html.indexOf("tab-bar") < html.indexOf("card__section-title"), "탭이 본문 아래에 있다");
});

test("**보고 있는 항목이 탭에 표시된다** — 응급은 주의사항 탭이다", () => {
  const { guidePreviewHtml } = box();

  /* 켜진 탭은 **그 단추 안에서** 찾는다 — 앞뒤 몇 글자를 떠서 보면 옆 단추의
     표시를 제 것으로 읽는다(이 저장소가 슬라이스로 여러 번 데었다). */
  const active = (html) => {
    const parts = html.split("<button").filter((p) => p.includes("tab-bar__btn--active"));
    assert.equal(parts.length, 1, "켜진 탭이 하나가 아니다");
    return parts[0];
  };

  assert.ok(active(guidePreviewHtml(SECTIONS, "life")).includes("생활관리"), "생활관리 탭이 안 켜졌다");

  /* 응급 문장을 볼 때도 환자에게는 「주의사항」 탭이다 */
  assert.ok(active(guidePreviewHtml(SECTIONS, "emergency")).includes("주의사항"), "응급일 때 주의사항 탭이 안 켜졌다");
});
...
test("**「문자 설정」은 다른 화면이다** — 원문·미리보기 두 칸이 아니다", () => {
  const { guideScreenHtml } = load("api", "session", "sms-plan", "patient-guide-cards", "guide-view");
...
test("**일주일 뒤는 켜진 채로 그려진다** — 끌 수 없는 회차다", () => {
  const { smsLeftHtml } = load("api", "session", "sms-plan", "patient-guide-cards", "guide-view");
...
test("**미리보기는 치환된 실제 발송본이다**", () => {
  const { smsRightHtml } = load("api", "session", "sms-plan", "patient-guide-cards", "guide-view");
...
test("**의원 템플릿이 아직 없다는 것은 그대로 말한다**", () => {
  /* 회차·문구를 담는 자리는 이제 서버에 있다(`PUT /guide/messages`). 그런데
     ...
     환자에게도 그 문구가 붙는 줄 안다. */
  const { SMS_NO_TEMPLATE, smsRightHtml } = load("api", "session", "sms-plan", "patient-guide-cards", "guide-view");
