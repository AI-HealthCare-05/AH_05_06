...
function box() {
  return load("api", "session", "patient-guide-cards", "patient-link-view", "guide-view");
}
...
test("**미리보기는 치환된 실제 발송본이다**", () => {
  const { smsRightHtml } = load("api", "session", "sms-plan", "patient-guide-cards", "patient-link-view", "guide-view");
...
test("**의원 템플릿이 아직 없다는 것은 그대로 말한다**", () => {
  /* 회차·문구를 담는 자리는 이제 서버에 있다(`PUT /guide/messages`). 그런데
     ...
     환자에게도 그 문구가 붙는 줄 안다. */
  const { SMS_NO_TEMPLATE, smsRightHtml } = load("api", "session", "sms-plan", "patient-guide-cards", "patient-link-view", "guide-view");
